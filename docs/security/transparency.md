...
| File | Purpose |
|------|---------|
| `rAIdio.bot-<tag>.cdx.json` | CycloneDX 1.5 SBOM. Every Rust crate, NPM package, Python dep, ComfyUI custom node, AI model, system tool. ~803 components, all version-pinned. |
...
- **A signed-binary attestation.** The hash files prove a published-
  and-installed pair match, not that the publisher is the legitimate
  vendor. Repository-compromise defense is GitHub's commit audit log
  and the public attention of users actually checking.
- **A telemetry feed.** rAIdio.bot does not phone home. The artefacts
  here are *static publications* the user fetches; the app itself
  does not contact this repo at runtime.
